...
run(() => {
  ...
  const draw = (n = 1) => {
    ...
    for (const location of locations) {
      ...
      if (checkBingo(location)) {
        notBingo.delete(location.board);
      }
    }
    draw(n - 1);
  };
  ...
  let notBingo = new Set(boards.map((_, i) => i));
  let drawIndex = -1;

  draw(5);
  while (notBingo.size > 1) {
    draw();
    if (numbers[drawIndex] == null) return;
  }
  const lastBoard = notBingo.values().next().value;
  while (notBingo.size > 0) {
    draw();
  }

  return { output: calculateScore(boards[lastBoard], numbers[drawIndex]) };
});
